...
- 前端：`flutter test` 覆盖模型、Repository、Provider、离线调度/Drift、ApiClient、同步服务、操作日志和主要页面 Widget；`queue_composer_test.dart` 直接断言插位规则（不依赖 Drift）；`offline_repository_test.dart` 含卡列表筛选口径断言（学新重学卡归学新、复习重学卡归复习）；`flutter analyze` 需保持无警告；`flutter test --coverage` 和 release Web 构建用于 CI 验证。
