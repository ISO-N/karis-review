...
- `StageRulerWidget`：接收当前 stage 与阶段分布，渲染九段记忆刻度；分布模式下有卡阶段按占比做青系渐变（`Color.lerp(jadeSoft, jade, ratio)`），无卡为发丝线灰，并为有卡阶段提供“新卡 · N 张”类无障碍标签。
- `KarisScrollBehavior`：全局滚动行为，桌面/Web 纵向滚动常驻可拖拽滚动条（`minThumbLength: 48` 兜底防卡片过多时滚动块过小），触屏保持默认 overlay，横向滚动不显示。
