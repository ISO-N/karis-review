...
///
/// 四个 Tab 由 StatefulShellRoute.indexedStack 保活，这里通过
/// StatefulNavigationShell 切换 branch：只改可见 index，页面 State 保留，
/// 不销毁重建、无转场动画，桌面端切换零卡顿。
///
/// 切换到统计页前先触发一次统计预取（方案 B：导航预取）——
/// 把可能已过期的网络同步提前到「点击 → 切换完成」的窗口内完成，
/// 进入统计页时数据往往已经就绪，页面内 initState 的加载请求会因
/// in-flight 防重入 / 5 分钟新鲜度缓存而直接跳过，零重复请求。
///
/// 预取对命中缓存的场景零开销；失败静默忽略（页面内仍有兜底加载）。
void goToTab(BuildContext context, WidgetRef ref, KarisNavItem item) {
  ...
  // Tab 页一定处于 StatefulShellRoute 内部；找不到 shell（如测试环境
  // 直接渲染单页）时退回 context.go 兜底。
