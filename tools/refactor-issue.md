...
- 登录恢复、首页、卡组页、统计页、卡片页都会触发全量 `/sync/bootstrap`，首页一次进入会并发触发两次全量下载。
- Bootstrap 每次下载全部卡组、全部卡片、全部历史复习日志。
